 ...
 * Text Domain:     all-post-list-block
 *
 ...
 */
function aplb_posts_block_init() {
	$dir = __DIR__;
	
	$script_asset_path = "$dir/build/index.asset.php";
	if ( ! file_exists( $script_asset_path ) ) {
		throw new Error(
			'You need to run `npm start` or `npm run build` for the "all-posts-list-block/post-list-block" block first.'
		);
	}
	...
	register_block_type(
		'all-posts-list-block/post-list-block',
		array(
			...
			'render_callback' => 'aplb_render_posts_block',
		)
	);

}
add_action( 'init', 'aplb_posts_block_init' );

function aplb_render_posts_block( $attributes ) {
	...
	if ( $postslist->have_posts() ) {
		while ( $postslist->have_posts() ) {
			$postslist->the_post();
			$file = get_stylesheet_directory() . '/all-post-list-template.php';
			?>
			...
			if ( file_exists( $file ) ) {
				...
				include 'templates/all-post-list-template.php';
			}
			?>
			</div>
			<?php
		...
		?>
		<div class="all-post-list">
			<?php echo esc_html( $attributes['noPostFoundText'] ); ?>
		</div>
		<?php
	}
	return ob_get_clean();
...
function aplb_test_paginate_links() {
		global $wp_query;
		$output    = '';
		$args_pagi = array(
			'base'      => add_query_arg( 'paged', '%#%' ),
...
class aplb_all_terms {
	...
	public function __construct() {
		...
		register_rest_route(
			...
			array(
				'methods'  => 'GET',
				'callback' => array( $this, 'aplb_get_all_terms' ),
			)
		);
	}

	public function aplb_get_all_terms( $object ) {
		$return     = array();
		// $args       = array(
		// 	'public'   => true,
		// 	'_builtin' => false,
